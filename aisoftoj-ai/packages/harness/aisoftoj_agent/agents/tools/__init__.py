...
def build_platform_tools(
    client: PlatformClient, textbook_trace_service: TraceService | None = None
) -> list[BaseTool]:
    return [
...
def build_agent_tools(
    ...
    tools = [*build_platform_tools(client, textbook_trace_service), *skill_tools]
    names = [tool.name for tool in tools]
    if len(names) != len(set(names)) or set(names) != AGENT_TOOL_NAMES:
        raise ValueError("agent tool set must contain the expected eight read-only tools")
    return tools
