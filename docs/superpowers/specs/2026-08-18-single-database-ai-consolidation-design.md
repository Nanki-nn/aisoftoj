...
the destination before dropping the now-empty `aisoftoj_ai` database.
...
The AI service remains stopped if the table move, row-count verification,
foreign-key verification, permission change, or Alembic check fails. Because
the user explicitly declined a backup, recovery before the old database is
dropped consists of renaming the tables back. The old database is dropped only
after all destination checks pass.
